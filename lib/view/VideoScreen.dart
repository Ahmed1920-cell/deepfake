  ...
  builder: (context, state) {
    ...
    if(state is ResultDone){
      ...
      return Padding(
        ...
        child: Column(
          children: [
            Detailsvideo(video: widget.videofile, name: name, result: state.Result, data: DateFormat.yMMMEd().format(DateTime.now())),
            ElevatedButton.icon(
              onPressed: () async {
                DeepModel data=DeepModel(video_path: widget.videofile!.path, name: name, result: state.Result, date: DateTime.now());
              ...
              label: Text("Done",style: TextStyle(fontSize: 18,color: Colors.white,fontWeight:FontWeight.bold),),
       ...
       return Center(
        child: TextWidget(text: "check your Internet", size: 18,color: Colors.black,),
      );
    }
  },
),
